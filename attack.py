...
def square_attack_linf(model, x, y, eps, n_iters, p_init, targeted, loss_type):
...
def square_attack_linf2(model1, model2, x, y, eps, n_iters, p_init, targeted, loss_type):
    """ The Linf square attack """
    np.random.seed(0)  # important to leave it here as well
    min_val, max_val = 0, 1 if x.max() <= 1 else 255
    #print(x.shape)
    ...
    # [c, 1, w], i.e. vertical stripes work best for untargeted attacks
    # init_delta = np.random.choice([-8/255, -4/255, -2/255, -1/255, 1/255, 2/255, 4/255, 8/255], size=[x.shape[0], c, 1, w])
    init_delta = np.random.choice([-eps, eps], size=[x.shape[0], c, 1, w])
    x_best = np.clip(x + init_delta, min_val, max_val)

    logits = model1.predict(x_best)
    loss_min1 = model1.loss(y, logits, targeted, loss_type=loss_type)
    margin_min1 = model1.loss(y, logits, targeted, loss_type='margin_loss')
    
    logits = model2.predict(x_best)
    loss_min2 = model2.loss(y, logits, targeted, loss_type=loss_type)
    margin_min2 = model2.loss(y, logits, targeted, loss_type='margin_loss')
    
    loss_min = (loss_min1 + loss_min2) / 2
    margin_min = (margin_min1 + margin_min2) / 2
    
    n_queries = np.ones(x.shape[0])  # ones because we have already used 1 query

    time_start = time.time()
    for i_iter in range(n_iters - 1):
        idx_to_fool = (margin_min1 > 0) | (margin_min2 > 0)
        if np.sum(idx_to_fool) == 0:
            break
        x_curr, x_best_curr, y_curr = x[idx_to_fool], x_best[idx_to_fool], y[idx_to_fool]
        loss_min_curr, margin_min_curr = loss_min[idx_to_fool], margin_min[idx_to_fool]
        margin_min_curr1 = margin_min1[idx_to_fool]
        margin_min_curr2 = margin_min2[idx_to_fool]
        deltas = x_best_curr - x_curr
        ...
        logits = model1.predict(x_new)
        loss1 = model1.loss(y_curr, logits, targeted, loss_type=loss_type)
        margin1 = model1.loss(y_curr, logits, targeted, loss_type='margin_loss')
        
        logits = model2.predict(x_new)
        loss2 = model2.loss(y_curr, logits, targeted, loss_type=loss_type)
        margin2 = model2.loss(y_curr, logits, targeted, loss_type='margin_loss')
        
        loss = (loss1 + loss2) / 2
        margin = (margin1 + margin2) / 2
        
        idx_improved = loss < loss_min_curr
        loss_min[idx_to_fool] = idx_improved * loss + ~idx_improved * loss_min_curr
        margin_min[idx_to_fool] = idx_improved * margin + ~idx_improved * margin_min_curr
        margin_min1[idx_to_fool] = idx_improved * margin1 + ~idx_improved * margin_min_curr1
        margin_min2[idx_to_fool] = idx_improved * margin2 + ~idx_improved * margin_min_curr2
        
        idx_improved = np.reshape(idx_improved, [-1, *[1]*len(x.shape[:-1])])
        x_best[idx_to_fool] = idx_improved * x_new + ~idx_improved * x_best_curr
        n_queries[idx_to_fool] += 1

        acc = ((margin_min1 > 0.0) & (margin_min2 > 0.0)).sum() / n_ex_total
        acc_corr = ((margin_min1 > 0.0) & (margin_min2 > 0.0)).mean()
        time_total = time.time() - time_start
        if acc == 0:
            break
...
def square_attack_linf_add(model1, model2, x, y, eps, n_iters, p_init, targeted, loss_type):
    """ The Linf square attack """
    np.random.seed(0)  # important to leave it here as well
    min_val, max_val = 0, 1 if x.max() <= 1 else 255
    c, h, w = x.shape[1:]
    ...
    for i_iter in range(n_iters - 1):
        ...
        margin_min_curr1 = margin_min1[idx_to_fool]
        margin_min_curr2 = margin_min2[idx_to_fool]
        loss_min_curr1 = loss_min1[idx_to_fool]
        loss_min_curr2 = loss_min2[idx_to_fool]
        deltas = x_best_curr - x_curr
        ...
        margin_min1[idx_to_fool] = idx_improved * margin1 + ~idx_improved * margin_min_curr1
        margin_min2[idx_to_fool] = idx_improved * margin2 + ~idx_improved * margin_min_curr2
        loss_min1[idx_to_fool] = idx_improved * loss1 + ~idx_improved * loss_min_curr1
        loss_min2[idx_to_fool] = idx_improved * loss2 + ~idx_improved * loss_min_curr2
        loss_min = (loss_min1 + loss_min2) / 2
        margin_min = (margin_min1 + margin_min2) / 2
...
def square_attack_linf_max(model1, model2, x, y, eps, n_iters, p_init, targeted, loss_type):
    """ The Linf square attack """
    np.random.seed(0)  # important to leave it here as well
    min_val, max_val = 0, 1 if x.max() <= 1 else 255
    c, h, w = x.shape[1:]
    ...
    logits1 = model1.predict(x_best)
    logits2 = model2.predict(x_best)
    logits = np.max((logits1, logits2), axis=0)
 
    loss_min = model1.loss(y, logits, targeted, loss_type=loss_type)
    margin_min = model1.loss(y, logits, targeted, loss_type='margin_loss')
    
    n_queries = np.ones(x.shape[0])  # ones because we have already used 1 query

    time_start = time.time()
    for i_iter in range(n_iters - 1):
        idx_to_fool = margin_min > 0 #(margin_min1 > 0) | (margin_min2 > 0)
        if np.sum(idx_to_fool) == 0:
            break
        x_curr, x_best_curr, y_curr = x[idx_to_fool], x_best[idx_to_fool], y[idx_to_fool]
        loss_min_curr, margin_min_curr = loss_min[idx_to_fool], margin_min[idx_to_fool]
        ...
        logits1 = model1.predict(x_new)
        logits2 = model2.predict(x_new)
        logits = np.max((logits1, logits2), axis=0)

        loss = model1.loss(y_curr, logits, targeted, loss_type=loss_type)
        margin = model1.loss(y_curr, logits, targeted, loss_type='margin_loss')
...
def square_attack_linf_lp(model1, model2, linear, x, y, eps, n_iters, p_init, targeted, loss_type):
    """ The Linf square attack """
    np.random.seed(0)  # important to leave it here as well
    min_val, max_val = 0, 1 if x.max() <= 1 else 255
    c, h, w = x.shape[1:]
    ...
    x_best = np.clip(x + init_delta, min_val, max_val)
    with torch.no_grad():
        logits = linear(torch.cat((torch.from_numpy(model1.predict(x_best)).cuda(), torch.from_numpy(model2.predict(x_best)).cuda()), dim=-1)).cpu().numpy()
    loss_min = myloss(y, logits, targeted, loss_type=loss_type)
    margin_min = myloss(y, logits, targeted, loss_type='margin_loss')
    ...
    for i_iter in range(n_iters - 1):
        ...
        x_new = np.clip(x_curr + deltas, min_val, max_val)
        with torch.no_grad():
            logits = linear(torch.cat((torch.from_numpy(model1.predict(x_new)).cuda(), torch.from_numpy(model2.predict(x_new)).cuda()), dim=-1)).cpu().numpy()
        loss = myloss(y_curr, logits, targeted, loss_type=loss_type)
        margin = myloss(y_curr, logits, targeted, loss_type='margin_loss')
...
if __name__ == '__main__':
    ...
    model = models.ModelPT('cifar10', 4096, 0.99)
    '''
    ...
    if args.model != 'pt_inception':
        x_test, y_test = data.datasets_dict[dataset](10000)
        print(x_test.shape)
    ...
    x_test, y_test = x_test[::10], y_test[::10] # x_test[:args.n_ex], y_test[:args.n_ex]
    print(x_test.shape)
