...
def cw(model,
    ...
    foolbox_model = PyTorchModel(model, bounds=(0, 1), num_classes=10, device='cuda:0', 
                  preprocessing=(norm_mean.view(1,3,1,1), 
                                  norm_std.view(1,3,1,1)), b=b)
    ...
    for i in range(len(X)):
        ...
        if adversarial is None:
            ...
            outputs.append(adversarial[None, :])
            # if model(adversarial[None, :]).argmax(axis=-1).cpu().numpy() == label:
            #     correct += 1
        else:
            # for j in range(4):
            adversarial2 = image + np.clip(adversarial - image, -eps, eps)
            ...
            outputs.append(adversarial2[None, :])
...
def cw_gen(model1,model2,
    ...
    if not HAVE_FOOLBOX:
        raise ImportError('Could not import FoolBox')
    foolbox_model = PyTorchModel(model1, model2, linear, bounds=(0, 1), num_classes=num_cls, device='cuda:0', 
                  preprocessing=(norm_mean.view(1,3,1,1), 
                                  norm_std.view(1,3,1,1)))
    attack = CarliniWagnerLIAttack(foolbox_model, distance=Linf)
    linf_distances = []
    ...
    for i in range(len(X)):
        ...
        adversarial = attack(image, label,
                             ...
                             learning_rate=learning_rate,
                             initial_const=initial_const,
                             tau_decrease_factor=tau_decrease_factor, ch=ch)
...
class CarliniWagnerLIAttack(Attack):
    ...
    def __call__(self, input_or_adv, label=None, unpack=True,
                 ...
                 initial_const=1e-2, abort_early=True, ch=0):
    ...
    def loss_function(cls, const, tau, a, x, logits, reconstructed_original,
        ...
        is_adv_loss_grad = a.backward_one(logits_diff_grad, x, ch=ch)
