...
class Model(abc.ABC):
    ...
    def _process_gradient(self, backward, dmdp):
        ...
        dmdx = backward(dmdp)
        dmdx = dmdx.astype(np.float32) # new
    ...
    def forward(self, inputs):
        ...
        """
        outs_combined, outs1, outs2 = self.forward(x)
        return outs_combined, outs1, outs2
    ...
    def forward_one(self, x):
        ...
        """
        outs_combined, outs1, outs2 = self.forward(x[np.newaxis])
        return np.squeeze(outs_combined, axis=0)
...
class DifferentiableModel(Model):
    ...
    def backward(self, gradient, inputs):
        ...
        """
        raise self.backward(gradient, x, ch)

    def backward_one(self, gradient, x, ch):
        """Backpropagates the gradient of some loss w.r.t. the logits through the underlying
        ...
        """
        return np.squeeze(self.backward(gradient[np.newaxis], x[np.newaxis], ch), axis=0)
