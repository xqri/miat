...
class Adversarial(object):
    ...
    def __init__(
        ...
        if unperturbed.ndim == 3:
            self._check_unperturbed_one()
        else:
            self._check_unperturbed()
            
    def _check_unperturbed_one(self):
        try:
    ...
    def _check_unperturbed(self):
        try:
            #print(self._unperturbed.shape)
            self.forward(self.__unperturbed)
        except StopAttack:
            # if a threshold is specified and the unperturbed input is
            # misclassified, this can already cause a StopAttack
            # exception
    ...
    def perturbed(self):
        ...
        #print("WQEWE", self.__best_adversarial)
    ...
    def __is_adversarial(self, x, predictions, in_bounds, original_cls):
        ...
        is_adversarial = self.__criterion.is_adversarial(
            predictions, original_cls #self.__original_class
        )
    ...
    def update_perturbed(self, x_best):
        self.__best_adversarial = x_best.copy()
    ...
    def forward_one(self, x, strict=True, return_details=False):
        ...
        is_adversarial, is_best, distance = self.__is_adversarial(
            x, predictions, in_bounds, self.__original_class
        )
    ...
    def forward(self, inputs, greedy=False, strict=True, return_details=False):
        ...
        predictions, predictions1, predictions2 = self.__model.forward(inputs)
        
        assert predictions.ndim == 2
        assert predictions.shape[0] == inputs.shape[0]
        
        #self.__best_adversarial = inputs.copy()
        ...
        for i in range(len(predictions)):
            ...
            is_adversarial, is_best, distance = self.__is_adversarial(
                inputs[i], predictions[i], in_bounds_i, self.__original_class[i]
            )
        ...
        '''
        return predictions, predictions1, predictions2, None #is_adversarial
    ...
    def backward(self, gradient, x=None, strict=True, ch=None):
        ...
        self._total_gradient_calls += 1
        gradient = self.__model.backward(gradient, x, ch)

        assert gradient.shape == x.shape
        return gradient
        
        
    def backward_one(self, gradient, x=None, strict=True, ch=None):
        """Interface to model.backward_one for attacks.
        ...
        self._total_gradient_calls += 1
        gradient = self.__model.backward_one(gradient, x, ch)
