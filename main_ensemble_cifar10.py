...
transform_train = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),#(0.4376821, 0.4437697, 0.47280442), (0.19803012, 0.20101562, 0.19703614)),#
])
...
trainloader = torch.utils.data.DataLoader(
    trainset, batch_size=416 + E, shuffle=True, num_workers=2)
...
net1 = ResNet_ensemble(depth=20, leaky_relu=False).cuda()
net2 = ResNet_ensemble(depth=20, leaky_relu=False).cuda()
...
optimizer_ori = optim.SGD(net_ori.parameters(), lr=0.001,
...
for i in range(2000):
    ...
    for n_iter, (image, label) in enumerate(trainloader):
        ...
        net_sa1 = models_sa.ModelPT(model_name='cifar10', _model=net1)
        net_sa2 = models_sa.ModelPT(model_name='cifar10', _model=net2)
        
        image_per5 = (image[bs*(right+sz_h*6):bs*(right+sz_h*8)] / denorm_std.view(1,3,1,1)) - denorm_mean.view(1,3,1,1)
        image_per5 = image_per5.numpy()
        label5 = label[bs*(right+sz_h*6):bs*(right+sz_h*8)].cpu().numpy()
        eps = 8/255
        y_target_onehot = utils_sa.dense_to_onehot(label5, n_cls=10)
        if n_iter % 2 == 1:
            n_queries, image_per5 = square_attack_linf2(net_sa1, net_sa2, image_per5, y_target_onehot, eps=eps, n_iters=800, p_init=0.8, targeted=False, loss_type='margin_loss')
        else:
            n_queries, image_per5 = square_attack_linf_max(net_sa1, net_sa2, image_per5, y_target_onehot, eps=eps, n_iters=800, p_init=0.8, targeted=False, loss_type='margin_loss')

        image_per5 = torch.from_numpy(image_per5).cuda().float()
        image_per5 = (image_per5 - norm_mean.view(1,3,1,1).cuda()) / norm_std.view(1,3,1,1).cuda()
        ...
        if n_iter % 3 == 0:
            image_per7 = cw_gen(net1, net2, image_per7, label[bs*(right+sz_h*8):bs*(right+sz_h*10)], max_iterations=30, norm_std=norm_std, norm_mean=norm_mean, num_cls=10, ch=0, linear=linear0)
        elif n_iter % 3 == 1:
            image_per7 = cw_gen(net1, net2, image_per7, label[bs*(right+sz_h*8):bs*(right+sz_h*10)], max_iterations=30, norm_std=norm_std, norm_mean=norm_mean, num_cls=10, ch=1, linear=linear0)
        else:
            image_per7 = cw_gen(net1, net2, image_per7, label[bs*(right+sz_h*8):bs*(right+sz_h*10)], max_iterations=30, norm_std=norm_std, norm_mean=norm_mean, num_cls=10, ch=2, linear=linear0)
            
        eps = torch.tensor(np.random.choice([8/255], image[bs*right:bs*(right+sz_h*6)].size(0))).cuda().float()        
        sz = eps.size(0) // 3

        if n_iter % 2 == 1:
            image_per3 = pgd_miat1(image[bs*right:bs*right+sz].cuda(), net1, net2, norm_std, norm_mean, denorm_std, denorm_mean, targets=label[bs*right:bs*right+sz], step_size=step_size, num_steps=40, epsil=eps[:sz], ch=0)
        else:
            image_per3 = pgd_miat1(image[bs*right:bs*right+sz].cuda(), net1, net2, norm_std, norm_mean, denorm_std, denorm_mean, targets=label[bs*right:bs*right+sz], step_size=step_size, num_steps=40, epsil=eps[:sz], ch=1)
        ...
        image_per6 = pgd_miat2(image[bs*right+sz*2:bs*right+sz*3].cuda(), net1, net2, None, norm_std, norm_mean, denorm_std, denorm_mean, targets=label[bs*right+sz*2:bs*right+sz*3], step_size=step_size, num_steps=20, epsil=eps, ch=1)
        ...
        image_per3 = torch.cat((image[bs*left:bs*right].cuda(), image_per3, image_per4, image_per6, image_per5, image_per7, image_per8))
        ...
        mask = (outs_max_rest > outs_max_tar * 1) 
        mask[bs*left:bs*right] = (outs_max_rest[bs*left:bs*right] > outs_max_tar[bs*left:bs*right] * 0.9) 
        mask = mask.detach()
