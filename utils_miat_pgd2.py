...
def pgd_miat2(inputs, net1, net2, linear, norm_std, norm_mean, denorm_std, denorm_mean, targets=None, step_size=2/255, num_steps=20, epsil=8./255, ch=0, n_iter=None):
    ...
    for ii in range(num_steps):
        ...
        if ch == 0:    
            fs = torch.cat((fs1, fs2), dim=-1)
            fs = linear.eval()(fs)
        else:
            fs = torch.max(fs1, fs2)
