...
def pgd_ori(inputs, net, norm_std, norm_mean, denorm_std, denorm_mean, targets=None, step_size=0.04, num_steps=20, epsil=5./255.*8, ch=0):
    ...
    pert_image = pert_image + (torch.rand(inputs.shape).cuda()-0.5) * 2 * epsil.view(-1,1,1,1) * scale 
    ...
    for ii in range(num_steps):
        ...
        fs_copy = fs.clone().detach()
        fs_copy[x_idx, targets] = -1e19
        _, tar = fs_copy.max(1)
        if ch == 0:
            loss_wrt_label = nn.CrossEntropyLoss()(fs, targets) - nn.CrossEntropyLoss()(fs, tar)
        else:
